...
# @app.route est un décorateur, le user invoque l'url ci-dessous
...
def login():
    if current_user.is_authenticated:
        return redirect(url_for('index'))
    form = LoginForm()
    # validate_on_submit(), prend tout les données et vérifie les validateurs
    # attaché aux champs et regarde si tout est bon
    if form.validate_on_submit():
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash('You have received an email for the instruction to reset your password')
        return redirect(url_for('login'))
...
# le formulaire géré par Flask-WTF youtube.colm 
